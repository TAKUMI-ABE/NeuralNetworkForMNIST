...
int main(void)
{
	...
	try {
		...
		*/
		...
		std::vector<double> u_3(MNIST_NUMBER_OF_OUTPUT_CLASS + BIAS_SIZE);
		...
		while (epoch_count < n_epoch)
		{
			while (count < train_img_info->get_n_img())
			{
				...
				for (int j = 0; j < delta_2.size(); j++) {
					for (int k = BIAS_SIZE; k < delta_3.size(); k++)
						//! ‚±‚±w_3‚ªj,k‚Å‚Í‚È‚­k,j‚Æ‚È‚Á‚Ä‚¢‚é‚©‚çvector‚Å‚Í‚Ç‚¤‚µ‚æ‚¤‚à‚È‚¢
